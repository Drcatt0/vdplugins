(function(f,L,g,t,B,C,c,d,F,D,G,k,O,E){"use strict";const H={saucenao:"https://saucenao.com",google:"https://images.google.com",yandex:"https://yandex.com/images"};var selectedSource="saucenao";const sourceChoices=Object.entries(H).map(function(e){let[a,i]=e;return{name:a,displayName:a,value:i}});
...
function SourceSettingsPage() {
    ...
    return t.React.createElement(t.ReactNative.ScrollView, {style: {flex: 1}},
        t.React.createElement(d.Forms.Search, {placeholder: "Search Source", onChangeText: setSearchTerm}),
        sourceChoices.filter(choice => choice.name.includes(searchTerm.toLowerCase()))
            .map(choice => t.React.createElement(d.Forms.FormRow, {
                label: choice.name.charAt(0).toUpperCase() + choice.name.slice(1), // Capitalize first letter
                trailing: () => t.React.createElement(d.Forms.FormRow.Arrow, null),
                onPress: () => { 
                    selectedSource = choice.name;
                    E.showToast(`Source set to ${choice.name}`, c.getAssetIDByName("check"));
                }
            }))
    );
}
